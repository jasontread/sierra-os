...
public class Menu extends Canvas implements MouseListener, MouseMotionListener{
  ...
  private Image offScreen=null;   // double buffering
  private Graphics offScreenG=null; // Image context for double buffering
  ...
  public void paint(Graphics g){
    ...
    if (offScreen==null){
      offScreen=createImage(Main.RIGHT_MARGIN, Main.DIM_Y);
      offScreenG=offScreen.getGraphics();
    }

    if (normal!=null){
      offScreenG.drawImage(normal,0,0,this);
    };

    // network status indicator
    switch(network_status){
      case 0: {offScreenG.drawImage(light,network.x1,network.y1,network.x2,network.y2,
        network.x1,network.y1,network.x2,network.y2,this);break;}
      case 1: {offScreenG.drawImage(normal,network.x1,network.y1,network.x2,network.y2,
        network.x1,network.y1,network.x2,network.y2,this);break;}
      case 2: {offScreenG.drawImage(bold,network.x1,network.y1,network.x2,network.y2,
        network.x1,network.y1,network.x2,network.y2,this);break;}
    }
    ...
    offScreenG.drawImage(light,but.x1,but.y1,but.x2,but.y2,but.x1,but.y1,but.x2,but.y2,this);

    // highlight button other than current
    if (!but.equals(mouseover) && mouseover!=null){
      offScreenG.drawImage(bold,mouseover.x1,mouseover.y1,mouseover.x2,mouseover.y2,
    ...
    if (small[thumbnail]!=null){
      offScreenG.drawImage(small[thumbnail],
        thumbnail_coord.x1,thumbnail_coord.y1,thumbnail_coord.x2-thumbnail_coord.x1,thumbnail_coord.y2-thumbnail_coord.y1,
      this);
    }else{
      offScreenG.setColor(new Color(emptyThColor));
      offScreenG.fillRect(thumbnail_coord.x1,thumbnail_coord.y1,
    ...
    if (network_status!=0){
      ...
      offScreenG.setColor(new Color(counterColor));
      offScreenG.drawString(str,counter_x-fm.stringWidth(str)/2,fm.getHeight()+counter_y);
    }
  ...
  private boolean parseSkindef(URL url, String skindef){
    URL wsk;
    try{
      wsk = new URL(url.getProtocol()+"://"+url.getHost()+url.getFile()+"skin/"+skindef);
