...
public class Server implements Runnable{
  ...
  final short RIGHT_MARGIN=100;                   // Menu width
  final short BOTTOM_MARGIN=50;                   // Colorpicker height
...
//        if (!runQuiet)System.out.println("** Pinging "+c+" with value "+CURRENT);
  ...
  synchronized void sendToAll(Object o){
    if (o instanceof Reset){
      // Jason: delete the bg image (emptymatrix) if used
      if (emptymatrix!=null) { emptymatrix = null; }
      
      eraseBgImage(((Reset)o).RGB);
      // Jason: don't need this
      //if (emptymatrix!=null)
