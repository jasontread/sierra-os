...
public class Archive implements Shape, Serializable{
  ...
  String send(){
    ...
    try{
      ...
      os.println("Date: " + msgDateFormat(today_date));
  ...
  private String msgDateFormat( Calendar senddate) {
      ...
      formatted = formatted + String.valueOf(senddate.get(Calendar.DAY_OF_MONTH)) + " ";
      formatted = formatted + Month[senddate.get(Calendar.MONTH)] + " ";

      if (senddate.get(Calendar.YEAR) < 99)
          formatted = formatted + String.valueOf(senddate.get(Calendar.YEAR) + 1900) + " ";
      else
          formatted = formatted + String.valueOf(senddate.get(Calendar.YEAR)) + " ";

      if (senddate.get(Calendar.HOUR_OF_DAY) < 10) formatted = formatted + "0";
      formatted = formatted + String.valueOf(senddate.get(Calendar.HOUR_OF_DAY)) + ":";
      if (senddate.get(Calendar.MINUTE) < 10) formatted = formatted + "0";
      formatted = formatted + String.valueOf(senddate.get(Calendar.MINUTE)) + ":";
      if (senddate.get(Calendar.SECOND) < 10) formatted = formatted + "0";
      formatted = formatted + String.valueOf(senddate.get(Calendar.SECOND)) + " ";
      if (senddate.getTimeZone().getRawOffset() < 0)
          formatted = formatted + "+";
      else
          formatted = formatted + "-";

      if (Math.abs( senddate.getTimeZone().getRawOffset()/1000 )/60 < 10)
        formatted = formatted + "0";
      formatted = formatted + String.valueOf(Math.abs( senddate.getTimeZone().getRawOffset()/1000 )/60);
      if (Math.abs( senddate.getTimeZone().getRawOffset()/1000 )%60 < 10)
        formatted = formatted + "0";
      formatted = formatted + String.valueOf(Math.abs( senddate.getTimeZone().getRawOffset()/1000 )%60);
